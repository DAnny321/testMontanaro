page 90202 "ATC_CP_Purchase Request"
{
    // version CP0

    Caption = 'Purchase Request',
                comment = 'ITA="Richiesta di acquisto"';
    PageType = Document;
            ...
            group(Generale)
            {
                ...
                field("ATC_CP_Expense Type"; Rec."ATC_CP_Expense Type")
                {
                    ...
                    var
                        ...
                        Mess_Err: Label 'For use Expence Type "%1" with default value is mandatory have a Default Vendor in General Setup',
                    ...
                    begin
                        ...
                        if (prType."Default Value" <> '') and (prSetup."Default Vendor" = '') then
                            Error(Mess_Err, Rec."ATC_CP_Expense Type");
                        CurrPage.PurchLines.PAGE.PrInitLine();
                        CurrPage.Update();
                ...
                field("ATC_CP_Exists Suggest Vendors"; Rec."ATC_CP_Exists Suggest Vendors")
                {
                    ApplicationArea = All;
                    Editable = false;
                    ToolTip = 'ATC_CP_Exists Suggest Vendors', Comment = 'ITA="Visualizza se esistono fornitori suggeriti"';
                }
                ...
                field("Requested Receipt Date"; Rec."Requested Receipt Date")
                {
                    ...
                    trigger OnValidate()
                    var
                        errMessLbl: Label '%1 is not valid', comment = 'ITA="%1 non può essere uguale o minore ad oggi"';
                    begin
                        if (Rec."Requested Receipt Date" <> 0D) and (Rec."Requested Receipt Date" <= WorkDate()) then
                            Error(errMessLbl, Rec.FieldCaption("Requested Receipt Date"));
                    end;
                }
                field("Shortcut Dimension 1 Code"; Rec."Shortcut Dimension 1 Code")
                {
                    ...
                    //AssocFind: Boolean;
                    //ErrorAssoc: Label 'User is not associated to selected Driver Dimension', comment = 'ITA="Utente non associato alla dimensione driver scelta"';
                    begin
                    ...
                    var
                        ...
                        AssocFind: Boolean;
                        ErrorAssocLbl: Label 'User is not associated to a Driver Dimension', comment = 'ITA="Utente non associato a nessuna dimensione driver"';
                    begin
                        clear(AssocFind);
                        Clear(PageDim);
                        TableDim.ClearMarks();
                ...
                {
                    ApplicationArea = All;
                    ToolTip = 'ATC_CP_Request on Behalf of', Comment = 'ITA="Specifica richiesta per conto di"';
                }
                ...
                field("ATC_CP_Create Order Vendor No."; Rec."ATC_CP_Create Order Vendor No.")
                {
                    ApplicationArea = All;
                    ShowMandatory = true;
                    ToolTip = 'ATC_CP_Create Order Vendor No.', Comment = 'ITA="Crea ordine su nr. fornitore"';
                    trigger OnValidate()
                    begin
                        CurrPage.Update();
                    end;
    ...
    actions
    {
        area(navigation)
        {
            group("Offe&rta")
            {
                ...
                {
                    Caption = 'Suggest Vendor List', comment = 'ITA="Lista fornitori suggeriti"';
                    ToolTip = 'Suggest Vendor List', comment = 'ITA="Lista fornitori suggeriti"';
                    ApplicationArea = All;
                    Image = Suggest;
    ...
    */
    ...
    begin
        ...
        ActivateFields();
        SetDocNoVisible();
        ATCCPGeneralSetup.CalcPageVisibility(Show1stLivel, Show2ndLivel, Show3rdLivel, ShowNoField);

        Clear(Dim1Mandatory);
        Clear(Dim2Mandatory);
        if ATCCPGeneralSetup.isGlobalDim1() then
            Dim1Mandatory := true
        else
            Dim2Mandatory := true;
    end;
    ...
    */
    ...
    begin
        ATCCPGeneralSetup.get();
        Rec.TestNoSeries();
        if NoSeriesManagement.SelectSeries(ATCCPGeneralSetup."Request Nos.", OldPurchaseHeader."No. Series", Rec."No. Series") then begin
            Rec.TestNoSeries();
